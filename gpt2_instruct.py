# Main code with revised cleaning function
import random
import numpy as np
...
# Step 2: Content Transformation Flow
...
# Step 3: Seed Instruction Generation Flow
...
# Step 4: Instruction Refinement Flow
...
# Step 5: Cleaning the refined instructions
def clean_text(text):
...
# Step 6: Remove Repeated Phrases
...
# Step 7: Monte Carlo Tree Search Integration for Few-Shot Learning and RLHF
...
# Step 8: Final formatting and ordering of steps
def format_final_instruction(instr):
    steps = [
        "1. Introduction",
        ...
        "4. Examples",
        "5. Key Takeaways",
        "6. Simplified Explanation for Beginners",
        "7. Conclusion"
    ]
    sentences = re.split(r'(?<=[.?!])\s+', instr)  # Split based on sentence endings
    formatted_instr = "\n".join([f"{step}: {sentence.strip()}" for step, sentence in zip(steps, sentences) if sentence])
    return formatted_instr

formatted_instructions = [format_final_instruction(instr) for instr in final_instructions]
print("Formatted Instructions:", formatted_instructions)  # Debug print

# Step 9: Print the refined instructions in the desired structured format
def print_instructions(instructions):
    for instr in instructions:
        print(instr)
        print("\n" + "-"*80 + "\n")

print_instructions(formatted_instructions)
